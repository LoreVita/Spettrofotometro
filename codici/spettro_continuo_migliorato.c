void spettro_continuo_migliorato(){
	...
	const char *NOME_FILE = "P-faretto_4.5V_filtro_rosso.txt";
	...
	if (filein.is_open()){
	 ...
	 double sigma_peak = 10; //sigma of searched peaks
	 double threshold = 10;  //threshold value in % for selected peaks (peaks with amplitude less than threshold*highest_peak/100 are ignored)
	 float offset = 20; //larghezza approssimativa del picco che si fitta
	 ...
	 float correction = 0.5; //termine per aggiustare la resa grafita del fit (rosso)
